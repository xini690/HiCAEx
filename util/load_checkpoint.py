...
def get_pretrained_vit_model(args, domain):
    ...
    if args.model and not args.eval:
        ...
        if 'global_tokens' in checkpoint['model']:
            print("Convert weights to RETFound weights", args.in_domains[0])
            checkpoint['model'] = hiCAEx_dict(checkpoint['model'], domain=args.in_domains[0], pretrain_weights=True)
