...
if __name__ == '__main__':
    args = SimpleNamespace(**{
        ...
        'drop_path': 0.1,
        'global_pool': True,
        "cfp_model" : "/data/zhengyi/mtf_moe/weight/RETFound_cfp_weights.pth",
        "oct_model" : "/data/zhengyi/mtf_moe/weight/RETFound_oct_weights.pth",
        'eval': False,
    ...
    cfp_img = torch.randn(2, 3, 224, 224)
    oct_img = torch.randn(2, 3, 224, 224)

    output= model(cfp_img, oct_img)

    print("Output shape:", output.shape)
    prpb=torch.nn.functional.softmax(output, dim=1)
    clas=output.argmax(dim=1)  # shape: [B]
    print("clas:", clas)
    print("prpb:", prpb)
    print("Output shape:", output.shape)
    print("Output:", output)
