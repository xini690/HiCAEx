...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--multimae_ckpt_path", default='RETFound_cfp_to_MultiMAE.pth', type=str,
        help="Path to MultiMAE checkpoint"
    )
    parser.add_argument(
        "--vit_ckpt_path", default='cfp_weights.pth', type=str,
        help="Path to converted ViT(MultiMAE) checkpoint"
    )
    args = parser.parse_args()
    
    print(f'Loading weights at {args.multimae_ckpt_path}')
    ckpt = torch.load(args.multimae_ckpt_path, map_location='cpu')
    # print(ckpt['model'].keys())
    # del ckpt['optimizer']
    # del ckpt['epoch']
    # del ckpt['scaler']
    # del ckpt['args']
    # del ckpt['loss_balancer']
